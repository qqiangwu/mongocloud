...
@Component
@Slf4j
public class MongoCommanderImpl implements MongoCommander {
    ...
    @Override
    public boolean removeShardFromCluster(final @Nonnull Instance router, @Nonnull final Instance shard) {
        try {
            @Cleanup
            val mongo = new MongoClient(router.getHostIP(), router.getPort());
            val db = mongo.getDatabase(DB_ADMIN);
